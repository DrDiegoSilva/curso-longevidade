...
def _campos_kit(gancho_bruto):
    ...
    reels = "".join(
        ...
        f'<textarea name="kit_reel_titulo" rows="2" {_CX}>{esc(x.get("titulo",""))}</textarea>'
        f'<label {_ROT}>Gancho (3 primeiros segundos)</label>'
        f'<textarea name="kit_reel_gancho" rows="2" {_CX}>{esc(x.get("gancho",""))}</textarea>'
    ...
    return (f'<details style="margin:14px 0;border:1px solid #d8ddd7;border-radius:8px;'
            ...
            f'<textarea name="kit_frase" rows="2" {_CX}>{esc(k.get("frase",""))}</textarea>'
